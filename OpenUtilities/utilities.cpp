...
namespace utilities {
	...
	void on_create(const game_object_script obj)
	{
		...
		else if (settings::fow::updatePos->get_bool() && obj->get_particle_target_attachment_object() && obj->get_particle_target_attachment_object()->is_valid() && obj->get_particle_target_attachment_object()->is_ai_base() && !obj->get_particle_target_attachment_object()->is_moving() && (!obj->get_particle_target_attachment_object()->get_path_controller() || obj->get_particle_target_attachment_object()->get_path_controller()->get_path_count() != 1) && !obj->get_particle_target_attachment_object()->is_visible() && !obj->get_particle_target_attachment_object()->is_hpbar_recently_rendered() && !obj->get_particle_target_attachment_object()->is_dead() && obj->get_particle_target_attachment_object()->get_position().is_valid())
		{
	...
	void on_issue_order(game_object_script& target, vector& pos, _issue_order_type& type, bool* process)
	{
		...
		{
			const auto spell = myhero->get_active_spell();
			if ((myhero->get_attack_cast_delay() > (0.066f + getPing()) && lastIssuedOrder > gametime->get_time() && lastAutoTime + myhero->get_attack_delay() - myhero->get_attack_cast_delay() - getPing() - 0.033f - (settings::corewalker::forcedownBuffer->get_bool() ? 0.0165f : 0.f) < gametime->get_time()) || (spell && spell->is_auto_attack() && attackFinishTime - getPing() - (settings::corewalker::forceBuffer->get_bool() ? 0.0165f : 0.f) >= gametime->get_time()))
			{
				*process = false;
				if (spell && spell->is_auto_attack())
					cancelBuffer = true;
